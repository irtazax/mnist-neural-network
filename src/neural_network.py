...
#Logits -> inputs of softmax (60000, 10) for mnist
def softmax(logits):
...
def classify(X, w1, w2):
    y_hat, _ = forward(X, w1, w2) #One row per label, one column per class. Row contains ten numbers between 0 and 1
    labels = np.argmax(y_hat, axis = 1) #Argmax returns the index of the largest element along an axis
    return labels.reshape(-1, 1)

#Implements cross-entropy loss for multiuclass classification, used in union with softmax
def loss(Y, y_hat):
...
def initialize_weights(n_input_variables, n_hidden_nodes, n_classes):
    w1_rows = n_input_variables + 1 # +1 for bias
    w1 = np.random.randn(w1_rows, n_hidden_nodes) * np.sqrt(1 / w1_rows)

    w2_rows = n_hidden_nodes + 1 # +1 for bias
